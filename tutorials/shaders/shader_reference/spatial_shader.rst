...
They can optionally be presented in world space by using the render mode ``world_vertex_coords``.
...
render_mode ``unshaded``. If no light function is written, Godot will use the material properties 
written to in the ``fragment()`` function to calculate the lighting for you (subject to the ``render_mode``).
...
|                                   | division by ``PI``.                                                    |
+-----------------------------------+------------------------------------------------------------------------+
| in float **SPECULAR_AMOUNT**      | ``2.0`` * :ref:`light_specular<class_Light3D_property_light_specular>` |
|                                   | property for :ref:`class_OmniLight3D` and  :ref:`class_SpotLight3D`.   |
|                                   | ``1.0`` for :ref:`class_DirectionalLight3D`.                           |
+-----------------------------------+------------------------------------------------------------------------+
